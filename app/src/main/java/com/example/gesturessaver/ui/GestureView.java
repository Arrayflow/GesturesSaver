...
import com.example.gesturessaver.logic.model.TouchPoint;
import com.example.gesturessaver.TouchUpEvent;
...
public class GestureView extends View {
    ...
    private Rect viewBoundary = new Rect();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (xx < 0) {
            xx = 0;
        } else if (xx > bitmap.getWidth()) {
            xx = bitmap.getWidth();
        }

        if (yy < 0) {
            yy = 0;
        } else if (yy > bitmap.getHeight()) {
            yy = bitmap.getHeight();
        }
...
//        Bitmap _bitmap = Bitmap.createBitmap(bitmap, rec.left, rec.top, rec.right - rec.left, rec.bottom - rec.top);
    ...
    public Bitmap getImage() {
        ...
        Bitmap _bitmap = Bitmap.createBitmap(bitmap, rec.left, rec.top, rec.right - rec.left, rec.bottom - rec.top);
        if (rec.left >= 3 && rec.top >= 3 && (2 * rec.right - rec.left + 6) <= bitmap.getWidth() && (2 * rec.bottom - rec.top + 6) <= bitmap.getHeight()) {
            _bitmap = Bitmap.createBitmap(bitmap, rec.left - 3, rec.top - 3, rec.right - rec.left + 6, rec.bottom - rec.top + 6);
